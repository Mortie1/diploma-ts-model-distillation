...
# Run N random subject-wise PAMAP2 folds (ssl_wearables protocol):
# each fold sets one subject as val and one as test (ordered pair), train = rest.
#
# Example:
#   MODEL_FAMILY=moment MODEL_TARGET=src.model.MomentClassificationAdapter MODEL_SIZE=base \
#   N_FOLDS=10 FOLD_SEED=42 scripts/run_pamap_ssl_10folds.sh
...
for row in "${FOLDS[@]}"; do
  ...
  echo "=== RUN fold=${fold_idx} val=${val_subj} test=${test_subj} ==="
  ...
  if [[ $rc -ne 0 ]]; then
    ...
    if [[ ! -f "$ckpt" ]]; then
      ...
      set +e
      python inference.py -cn=fm_inference \
        datasets=pamap2 \
        model._target_="${MODEL_TARGET}" \
        model.model_size="${MODEL_SIZE}" \
        model.n_classes="${N_CLASSES}" \
